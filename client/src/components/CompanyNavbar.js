...
const CompanyNavbar = () => {
    ...
    const [companyDetailsHash, setcompanyDetailsHash] = useState();
    const [companyName, setcompanyName] = useState();
    const [companyProfilePicHash, setcompanyProfilePicHash] = useState();
    ...
    useEffect(() => {
      ...
      const retrieveCompanyInfo = async () => {
        ...
        setcompanyDetailsHash(companyDetails.value["company_profile_Id"]);
      }
      if(wallet){
        if(!balance)
            retrieveBalance();
        if(!companyDetailsHash)
            retrieveCompanyInfo();
      }
    }, [wallet])

    useEffect(() => {
        fetch("https://ipfs.io/ipfs/QmaMAV5Begk9CGp6Pugk4gjrwiTAJBSuxrHKiCt1qLcmCz")
        .then(res => res.json())
        .then((result) => {
            console.log(result)
            setcompanyName(result["name"]);
            setcompanyProfilePicHash(result["linkedIn"]);
        })
        .catch(err => { throw err });
    }, [companyDetailsHash])
                ...
                >
                    ...
                    <List className="mx-auto" style={{ width: '90%' }}>
                        ...
                        <div className=" py-3 ps-4 my-2" style={{ borderRadius: '5px', backgroundColor: 'rgb(25, 33, 48)' }}>
                            <h6 className="font15 menu-item-color mb-1">{companyName}</h6>
                            <h6 className="font10 m-0">{companyProfilePicHash}</h6>
                        </div>
